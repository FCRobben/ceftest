...
CefRefPtr<CefResourceRequestHandler> SimpleHandler::GetResourceRequestHandler(
    ...
    bool is_navigation, bool is_download, const CefString& request_initiator, bool& disable_default_handling)
{
    auto strUrl = request->GetURL().ToString();
    if (0 == strUrl.find("http://local.test.cn"))
    {
       if (!m_spResourceRequestHandler)
            m_spResourceRequestHandler = new SimpleResourceRequestHandler();
        return m_spResourceRequestHandler;
    }
    return nullptr;
}
